...
//import { RecaptchaComponent } from '@node_modules/ng-recaptcha';
//import { PropertyGridEditorCollection } from 'survey-creator-core';

const CUSTOM_TYPE = "reCaptcha";
...
export class CustomCaptchaComponent extends QuestionAngular<QuestionReCaptchaModel> {
  question: any;

  onCaptchaResolved(token: string | null) {
    this.model.value = token;
    console.log('reCAPTCHA token set to question value:', token);
  }
}
AngularComponentFactory.Instance.registerComponent(CUSTOM_TYPE + "-question", CustomCaptchaComponent);
export class QuestionReCaptchaModel extends Question {
  getType() {
    return CUSTOM_TYPE;
  }
  ...
  get reCaptchaType() {
    return this.getPropertyValue("reCaptchaType");
  }
  set reCaptchaType(val) {
    this.setPropertyValue("reCaptchaType", val);
  }
}

ElementFactory.Instance.registerElement(CUSTOM_TYPE, (name) => {
  return new QuestionReCaptchaModel(name);
},false);
...
Serializer.addClass(
  ...
  [{
    name: "reCaptchaType",
    default: "v2-checkbox", // یک مقدار پیش‌فرض معنادار
    choices: ["v2-checkbox", "v2-invisible", "v3"],
    category: "general",
    displayName: "اعتبارسنجی کپچا",
    visibleIndex: 2 // After the Name and Title
  }, {
    name: "disableAlpha:boolean",
    dependsOn: "reCaptchaType",
    visibleIf: function (obj) {
      return obj.reCaptchaType === "captcha";
    },
    category: "general",
    visibleIndex: 3 // After the Name, Title, and Color Picker type
  }],
  function () {
    return new QuestionReCaptchaModel("");
  },
  "question"
);
...
//   fit: function (prop) {
//     return prop.type === "captcha";
//   },
//   getJSON: function () {
//     return {
//       type: CUSTOM_TYPE,
//       colorPickerType: "reCaptchaType"
//     };
//   }
// });
